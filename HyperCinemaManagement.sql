...
-- ===================================================
-- MIGRATION: Bổ sung cột `created_at` vào bảng `promotion`
-- (phục vụ sắp xếp danh sách voucher theo ngày tạo - Voucher Management Req 1.1)
-- An toàn khi chạy lại: chỉ thêm cột nếu chưa tồn tại.
-- ===================================================
SET @add_promotion_created_at := (
    SELECT IF(
        EXISTS(
        ...
        'SELECT 1',
        'ALTER TABLE `promotion` ADD COLUMN `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP'
    )
);
PREPARE stmt FROM @add_promotion_created_at;
EXECUTE stmt;
